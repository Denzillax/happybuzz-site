...
export default function LegalLayout({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#F4F4F2' }}>
      <header style={{ background: '#fff', borderBottom: '1px solid #E4E0D8', padding: '14px 24px' }}>
      ...
      <footer style={{ background: '#fff', borderTop: '1px solid #E4E0D8', padding: '14px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 8 }}>
          <span style={{ fontSize: 12, color: '#8A8580' }}>© 2026 BEEDARO, eine Marke von MOQRO by Denis Mihaljevic · CHE-237.380.784</span>
          <span style={{ display: 'flex', gap: 16 }}>
            <Link href="/terms" style={{ fontSize: 12, color: '#8A8580', textDecoration: 'none' }}>AGB</Link>
            <Link href="/privacy" style={{ fontSize: 12, color: '#8A8580', textDecoration: 'none' }}>Datenschutz</Link>
            <Link href="/imprint" style={{ fontSize: 12, color: '#8A8580', textDecoration: 'none' }}>Impressum</Link>
          </span>
        </div>
      </footer>
    </div>
