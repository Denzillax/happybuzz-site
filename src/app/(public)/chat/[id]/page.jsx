    ...
    <div style={{ fontFamily: fonts.body, background: colors.surface, flex: 1, minHeight: 0, display: "flex", flexDirection: "row", color: colors.dark }}>
                  ...
                  <div style={{ maxWidth: "80%", padding: "12px 16px", borderRadius: 20, background: "#FFF9E6", border: `1.5px solid ${colors.dark}`, borderBottomRightRadius: isMe ? 4 : 16, borderBottomLeftRadius: isMe ? 16 : 4 }}>
                    ...
                    {showActions && (
                      <div style={{ display: "flex", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
                        <button onClick={() => acceptOffer(amount)} disabled={sending} style={{ padding: "7px 14px", borderRadius: 20, border: "none", background: "#CEF6E8", color: "#1D1D1D", fontSize: 12, fontWeight: 700, fontFamily: fonts.body, cursor: "pointer" }}>Annehmen</button>
      ...
      {conv?.listing && (
        <aside className="chat-info" style={{ width: 280, flexShrink: 0, borderLeft: `1px solid ${colors.borderLt}`, background: colors.surface, padding: 18, overflowY: "auto", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <Link href={`/listing/${conv.listing.id}`} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "10px 14px", borderRadius: radius.full, background: "#CEF6E8", color: "#1D1D1D", fontSize: 13, fontWeight: 700, fontFamily: fonts.body, textDecoration: "none" }}>Zum Inserat</Link>
          {otherUser?.id && (
