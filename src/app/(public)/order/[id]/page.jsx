...
function CopyBtn({ text }) {
  ...
  return (
    <button onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 1500); }}
      style={{ background: "none", border: "none", cursor: "pointer", padding: 4, color: copied ? "#50804F" : colors.muted, display: "flex", alignItems: "center", transition: "color .2s" }}
      title="Kopieren">
    ...
    <div>
      ...
      {items.length > 0 ? (
        <div style={{ background: "#F3F3FF", borderRadius: 20, padding: "14px 16px", marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 700, fontSize: 16, paddingTop: 10, marginTop: 4, borderTop: "2px solid rgba(29,29,29,.16)" }}>
      ...
      {serviceQrPayload(purchaseId, total, sellerProfile) && (
        <div style={{ display: "flex", gap: 14, padding: "14px 16px", background: "#fff", borderRadius: 20, border: `1px solid ${colors.borderLt}`, marginBottom: 12 }}>
  ...
  return (
    <div style={{ fontFamily: fonts.body, background: "var(--bd-grund)", minHeight: "100vh", color: colors.dark }}>
        ...
        <div onClick={() => setSalePopup(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.55)", zIndex: 10000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, padding: "28px 26px", maxWidth: 360, width: "100%", textAlign: "center", fontFamily: fonts.body }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <Link href="/hive" onClick={() => setSalePopup(null)} style={{ flex: 1, padding: "12px 0", borderRadius: 20, background: K.petrol, color: "#fff", fontSize: 14, fontWeight: 800, textDecoration: "none" }}>Belohnungen einlösen</Link>
              <button onClick={() => setSalePopup(null)} style={{ padding: "12px 18px", borderRadius: 20, border: `1.5px solid ${colors.border}`, background: "#fff", color: colors.muted, fontSize: 13, cursor: "pointer", fontFamily: fonts.body }}>Schliessen</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bd-seite">
        ...
        <div style={{ fontSize: 10, fontWeight: 700, fontFamily: MONO, letterSpacing: ".18em", textTransform: "uppercase", color: K.petrol, marginBottom: 8 }}>Bestellung</div>
        ...
        {/* Produkt-Card */}
        <div style={{ position: "relative", background: "#fff", borderRadius: 20, border: "1px solid #1D1D1D", padding: 20, marginBottom: 16, display: "flex", gap: 16, alignItems: "center" }}>
          {stempel && <span className="bd-fx-stamp" style={{ position: "absolute", right: 18, top: 14 }}>{isRental ? "Gemietet" : "Gekauft"}</span>}
          <div style={{ width: 80, height: 80, borderRadius: 20, overflow: "hidden", background: colors.cream, border: "1px solid #1D1D1D", flexShrink: 0 }}>
        ...
        <div className="order-grid" style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 20, alignItems: "start" }}>
          ...
          <div>
            ...
            {isFinished && (
              <div style={{ background: "#DBF5F0", borderRadius: 20, padding: "16px 20px", marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: 13, color: "#50804F" }}>Fertig.</div>
                </div>
              </div>
            )}
            ...
            {/* Aktions-Box */}
            {!isFinished && (
              <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #1D1D1D", padding: 20, marginBottom: 16 }}>
                ...
                {isService && isBuyer && p.status === "confirmed" && (
                  <div style={{ textAlign: "center", padding: 16 }}><Clock size={32} color="#1D1D1D" style={{ marginBottom: 8 }} /><p style={{ fontSize: 15, fontWeight: 700, margin: "0 0 4px" }}>Termin bestaetigt</p><p style={{ fontSize: 13, color: "#5B626C", margin: 0 }}>Der Anbieter erledigt den Auftrag und schickt dir danach die Rechnung.</p></div>
                )}
                ...
                {!isService && isBuyer && (p.status === "confirmed" || p.status === "pending_payment") && (
                  <div>
                    ...
                    <button onClick={() => doAction(markAsPaid, p.id, user.id)} disabled={acting} style={{ width: "100%", padding: 14, borderRadius: 20, border: "1px solid #1D1D1D", background: K.petrol, color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: fonts.body }}>{acting ? "Wird gespeichert..." : "Ich habe bezahlt"}</button>
                  </div>
                )}
                {!isService && isBuyer && (p.status === "payment_pending" || p.status === "payment_marked") && (
                  <div style={{ textAlign: "center", padding: 10 }}><Clock size={28} color="#8A5A00" style={{ marginBottom: 8 }} /><p style={{ fontSize: 14, fontWeight: 700, margin: "0 0 4px" }}>Zahlung markiert</p><p style={{ fontSize: 13, color: colors.muted, margin: 0 }}>Der Verkäufer prüft deine Zahlung.</p></div>
                )}
                {!isService && isSeller && (p.status === "confirmed" || p.status === "pending_payment" || p.status === "payment_pending" || p.status === "payment_marked") && (
                  <div>
                    ...
                    <button onClick={() => doAction(confirmPayment, p.id, user.id)} disabled={acting} style={{ width: "100%", padding: 14, borderRadius: 20, border: "1px solid #1D1D1D", background: K.petrol, color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: fonts.body }}>{acting ? "Wird gespeichert..." : "Zahlung erhalten"}</button>
                  </div>
                )}
                  ...
                  <div>
                    ...
                    {!wirdAbgeholt && (
                      <>
                        ...
                        <div style={{ padding: 12, background: K.sand, borderRadius: 20, border: `1px solid ${K.ink}22`, marginBottom: 14, fontSize: 13, lineHeight: 1.5 }}>
                        ...
                        {!showTracking ? (
                          <button onClick={() => setShowTracking(true)} style={{ width: "100%", padding: 14, borderRadius: 20, border: "1px solid #1D1D1D", background: K.petrol, color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: fonts.body }}>Als versendet markieren</button>
                        ) : (
                          <div>
                            <label style={{ fontSize: 12, fontWeight: 700, color: colors.muted, display: "block", marginBottom: 4 }}>Sendungsnummer (optional)</label>
                            <input value={trackingInput} onChange={e => setTrackingInput(e.target.value)} placeholder="z.B. 996000648206316303" style={{ width: "100%", padding: "10px 12px", borderRadius: 20, border: "1px solid #1D1D1D", fontSize: 14, fontFamily: fonts.body, outline: "none", boxSizing: "border-box", marginBottom: 10 }} />
                            <button onClick={() => doAction(markAsShipped, p.id, user.id, trackingInput)} disabled={acting} style={{ width: "100%", padding: 14, borderRadius: 20, border: "1px solid #1D1D1D", background: K.petrol, color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: fonts.body }}>{acting ? "Wird gespeichert..." : "Versand bestätigen"}</button>
                          </div>
                        )}
                      </>
                    )}
                    {wirdAbgeholt && (
                      <button onClick={() => doAction(markAsPickedUp, p.id, user.id)} disabled={acting} style={{ width: "100%", padding: 14, borderRadius: 20, border: "1px solid #1D1D1D", background: K.petrol, color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: fonts.body }}>{acting ? "Wird gespeichert..." : "Als übergeben markieren"}</button>
                    )}
                  </div>
                )}
                {!isService && isBuyer && p.status === "paid" && (
                  <div style={{ textAlign: "center", padding: 10 }}><Truck size={28} color="#1D1D1D" style={{ marginBottom: 8 }} /><p style={{ fontSize: 14, fontWeight: 700, margin: "0 0 4px" }}>Bezahlt</p><p style={{ fontSize: 13, color: colors.muted, margin: 0 }}>Der Verkäufer bereitet den Versand vor.</p></div>
                )}
                ...
                {isService && isSeller && p.status === "paid" && (
                  <div>
                    ...
                    <button onClick={() => doAction(completeTransaction, p.id, user.id)} disabled={acting} style={{ width: "100%", padding: 14, borderRadius: 20, border: "1px solid #1D1D1D", background: K.petrol, color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: fonts.body }}>{acting ? "Wird gespeichert..." : "Auftrag abschliessen"}</button>
                  </div>
                )}
                {isService && isBuyer && p.status === "paid" && (
                ...
                {isBuyer && (p.status === "shipped" || p.status === "picked_up") && (
                  <div>
                    ...
                    {trackingEvent && (
                      <div style={{ padding: 12, background: K.sand, borderRadius: 20, border: `1px solid ${K.ink}22`, marginBottom: 14, fontSize: 13 }}>
                    ...
                    <button onClick={() => doAction(confirmDelivery, p.id, user.id)} disabled={acting} style={{ width: "100%", padding: 14, borderRadius: 20, border: "1px solid #1D1D1D", background: K.petrol, color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: fonts.body }}>{acting ? "Wird gespeichert..." : "Empfang bestätigen"}</button>
                  </div>
                )}
                {isSeller && (p.status === "shipped" || p.status === "picked_up") && (
                  <div style={{ textAlign: "center", padding: 10 }}><Truck size={28} color="#1D1D1D" style={{ marginBottom: 8 }} /><p style={{ fontSize: 14, fontWeight: 700, margin: "0 0 4px" }}>{p.status === "shipped" ? "Versendet" : "Übergeben"}</p><p style={{ fontSize: 13, color: colors.muted, margin: 0 }}>Warte auf Empfangsbestätigung {isRental ? "des Mieters" : "des Käufers"}.</p></div>
                )}
                ...
                {isRental && isBuyer && p.status === "delivered" && (
                  <div>
                    ...
                    <button onClick={() => doAction(markAsReturned, p.id, user.id)} disabled={acting} style={{ width: "100%", padding: 14, borderRadius: 20, border: "1px solid #1D1D1D", background: K.petrol, color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: fonts.body }}>{acting ? "Wird gespeichert..." : "Ich habe zurückgegeben"}</button>
                  </div>
                )}
                {isRental && isSeller && p.status === "delivered" && (
                  <div style={{ textAlign: "center", padding: 10 }}>
                    <Package size={28} color="#1D1D1D" style={{ marginBottom: 8 }} />
                  ...
                  <div>
                    ...
                    {!showDamageForm ? (
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => doAction(confirmReturn, p.id, user.id, depositAmount)} disabled={acting} style={{ flex: 1, padding: 14, borderRadius: 20, border: "1px solid #1D1D1D", background: K.petrol, color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: fonts.body }}>{acting ? "..." : "Alles OK, Kaution zurück"}</button>
                        <button onClick={() => setShowDamageForm(true)} style={{ flex: 1, padding: 14, borderRadius: 20, border: "1px solid #1D1D1D", background: "#fff", color: colors.dark, fontSize: 14, fontWeight: 700, cursor: "pointer", fontFamily: fonts.body }}>Schaden melden</button>
                      </div>
                    ) : (
                      <div>
                        <label style={{ fontSize: 12, fontWeight: 700, color: colors.muted, display: "block", marginBottom: 4 }}>Schadenshöhe (CHF)</label>
                        <input type="number" value={damageAmount} onChange={e => setDamageAmount(e.target.value)} placeholder="z.B. 20" style={{ width: "100%", padding: "10px 12px", borderRadius: 20, border: "1px solid #1D1D1D", fontSize: 14, fontFamily: fonts.body, outline: "none", boxSizing: "border-box", marginBottom: 10 }} />
                        <label style={{ fontSize: 12, fontWeight: 700, color: colors.muted, display: "block", marginBottom: 4 }}>Beschreibung</label>
                        <textarea value={damageDesc} onChange={e => setDamageDesc(e.target.value)} placeholder="Was ist beschädigt?" rows={2} style={{ width: "100%", padding: "10px 12px", borderRadius: 20, border: `1.5px solid ${colors.border}`, fontSize: 13, fontFamily: fonts.body, outline: "none", boxSizing: "border-box", resize: "vertical", marginBottom: 10 }} />
                        ...
                        {damageFiles.length > 0 && (
                          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
                            {damageFiles.map((f, i) => (
                              <div key={i} style={{ width: 56, height: 56, borderRadius: 20, overflow: "hidden", border: `1px solid ${colors.border}` }}>
                        ...
                        <div style={{ display: "flex", gap: 8 }}>
                          <button onClick={async () => {
                            ...
                            setActing(false);
                          }} disabled={acting || !damageAmount || !damageDesc} style={{ flex: 1, padding: 14, borderRadius: 20, border: "none", background: "#c62828", color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: fonts.body }}>{acting ? "Wird hochgeladen..." : "Schaden melden"}</button>
                          <button onClick={() => { setShowDamageForm(false); setDamageFiles([]); }} style={{ padding: "14px 20px", borderRadius: 20, border: "1px solid #1D1D1D", background: "#fff", color: colors.muted, fontSize: 13, cursor: "pointer", fontFamily: fonts.body }}>Abbrechen</button>
                        </div>
                      </div>
                    )}
                  </div>
                ...
                {isRental && isBuyer && p.status === "return_pending" && (
                  <div style={{ textAlign: "center", padding: 10 }}><Clock size={28} color="#8A5A00" style={{ marginBottom: 8 }} /><p style={{ fontSize: 14, fontWeight: 700, margin: "0 0 4px" }}>Rückgabe markiert</p><p style={{ fontSize: 13, color: colors.muted, margin: 0 }}>Der Vermieter prüft den Zustand des Artikels.</p></div>
                )}
                ...
                {isRental && isBuyer && p.status === "damage_reported" && (
                  <div>
                    ...
                    {p.damage_photos && p.damage_photos.length > 0 && (
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
                        {p.damage_photos.map((url, i) => (
                          <a key={i} href={url} target="_blank" rel="noopener" style={{ width: 80, height: 80, borderRadius: 20, overflow: "hidden", border: `1px solid ${colors.border}`, display: "block" }}>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      <button onClick={() => doAction(acceptDamage, p.id, user.id)} disabled={acting} style={{ flex: 1, padding: 14, borderRadius: 20, border: "1px solid #1D1D1D", background: K.petrol, color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: fonts.body }}>{acting ? "..." : "Akzeptieren"}</button>
                    </div>
                  </div>
                )}
                  ...
                  <div style={{ textAlign: "center", padding: 10 }}>
                    ...
                    {p.damage_photos && p.damage_photos.length > 0 && (
                      <div style={{ display: "flex", gap: 6, justifyContent: "center", flexWrap: "wrap" }}>
                        {p.damage_photos.map((url, i) => (
                          <a key={i} href={url} target="_blank" rel="noopener" style={{ width: 56, height: 56, borderRadius: 20, overflow: "hidden", border: `1px solid ${colors.border}`, display: "block" }}>
                            <img src={url} alt={`Schaden ${i + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                          </a>
                        ))}
                      </div>
                ...
                {isRental && isSeller && p.status === "returned" && (
                  <div>
                    ...
                    <a href={`/order/${p.id}/invoice?type=deposit`} target="_blank" rel="noopener" style={{ display: "block", textAlign: "center", padding: 14, borderRadius: 20, border: `1.5px solid #50804F`, background: "#DBF5F0", color: "#50804F", fontSize: 14, fontWeight: 700, textDecoration: "none", fontFamily: fonts.body, marginBottom: 10 }}>Kautions-Rechnung (QR) ansehen</a>
                ...
                {isRental && isBuyer && p.status === "returned" && (
                  <div style={{ textAlign: "center", padding: 10 }}>
                    <CreditCard size={28} color="#50804F" style={{ marginBottom: 8 }} />
                    ...
                    {depositAmount > 0 && (
                      <>
                        ...
                        <div style={{ padding: 12, background: K.sand, borderRadius: 20, border: `1px solid ${K.ink}22`, marginBottom: 12, fontSize: 13, textAlign: "left" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}><span>Kaution</span><span>CHF {fmtCHF(depositAmount)}</span></div>
                          {parseFloat(p.damage_amount || 0) > 0 && (
                            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, color: "#c62828" }}><span>Schaden</span><span>- CHF {fmtCHF(p.damage_amount)}</span></div>
                          )}
                        ...
                        <a href={`/order/${p.id}/invoice?type=deposit`} target="_blank" rel="noopener" style={{ display: "block", textAlign: "center", padding: 12, borderRadius: 20, border: `1.5px solid #50804F`, background: "#DBF5F0", color: "#50804F", fontSize: 13.5, fontWeight: 700, textDecoration: "none", fontFamily: fonts.body }}>Kautions-Rechnung (QR) ansehen</a>
                      </>
                    )}
                  </div>
                )}
          ...
          <div>
                    ...
                    <div style={{ marginTop: 10, display: "grid", gap: 6 }}>
                      {[
                        ...
                        <button key={opt.key} disabled={addrSaving}
                          ...
                          style={{ textAlign: "left", padding: "8px 10px", fontSize: 12, lineHeight: 1.4, background: "#fff", border: "1px solid #1D1D1D", borderRadius: 20, cursor: "pointer", fontFamily: fonts.body }}>
              ...
              {isRental && depositAmount > 0 && (
                <SidebarSection icon={CreditCard} title="Kaution">
                  <div style={{ lineHeight: 1.5 }}>
                    ...
                    {p.status === "completed" && <><br /><span style={{ fontSize: 12, color: "#50804F", fontWeight: 600 }}>Zurückerstattet</span></>}
                    {p.status === "returned" && <><br /><span style={{ fontSize: 12, color: "#8A5A00", fontWeight: 600 }}>Rückerstattung ausstehend</span></>}
            ...
            {isBuyer && p.seller?.iban && (
              <div id="payment-info" style={{ background: "#fff", borderRadius: 20, border: "1px solid #1D1D1D", padding: 20, marginBottom: 16 }}>
                ...
                <Link href={`/order/${p.id}/invoice`} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 16, padding: "12px 16px", borderRadius: 20, background: K.petrol, color: "#fff", fontSize: 13, fontWeight: 700, textDecoration: "none", fontFamily: fonts.body }}><FileText size={15} /> QR-Rechnung anzeigen</Link>
              </div>
            )}
            ...
            {!(isBuyer && p.seller?.iban) && (
              <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #1D1D1D", padding: 20, marginBottom: 16 }}>
                ...
                <Link href={`/order/${p.id}/invoice`} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "12px 16px", borderRadius: 20, background: K.petrol, color: "#fff", fontSize: 13, fontWeight: 700, textDecoration: "none", fontFamily: fonts.body }}><FileText size={15} /> QR-Rechnung anzeigen</Link>
              </div>
            )}

            {/* Hilfe-Box */}
            <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #1D1D1D", padding: 20 }}>
