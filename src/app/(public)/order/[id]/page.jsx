  ...
  return (
    <div>
      ...
      {serviceQrPayload(purchaseId, total, sellerProfile) && (
        <div style={{ display: "flex", gap: 14, padding: "14px 16px", background: "#fff", borderRadius: 12, border: `1px solid ${colors.borderLt}`, marginBottom: 12 }}>
    ...
    <div style={{ fontFamily: fonts.body, background: "var(--bd-grund)", minHeight: "100vh", color: colors.dark }}>
      ...
      {salePopup && (
        <div onClick={() => setSalePopup(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.55)", zIndex: 10000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 12, padding: "28px 26px", maxWidth: 360, width: "100%", textAlign: "center", fontFamily: fonts.body }}>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "center", margin: "0 0 14px", flexWrap: "wrap" }}>
              {salePopup.pollen > 0 && <span style={{ fontSize: 13, fontWeight: 800, color: "#50804F", background: "#50804F14", padding: "6px 12px", borderRadius: 12 }}>+{salePopup.pollen} Pollen</span>}
      ...
      <div className="bd-seite">
        ...
        <div className="order-grid" style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 20, alignItems: "start" }}>
          ...
          <div>
            {/* Status-Banner */}
            {isFinished && (
              <div style={{ background: "#E8F5E9", borderRadius: 12, padding: "16px 20px", marginBottom: 16, display: "flex", alignItems: "center", gap: 10 }}>
              ...
              <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E5E8EC", padding: 20, marginBottom: 16 }}>
                ...
                {isBuyer && (p.status === "shipped" || p.status === "picked_up") && (
                  <div>
                    ...
                    {trackingEvent && (
                      <div style={{ padding: 12, background: K.sand, borderRadius: 12, border: `1px solid ${K.ink}22`, marginBottom: 14, fontSize: 13 }}>
                      ...
                      <div>
                        ...
                        {damageFiles.length > 0 && (
                          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
                            {damageFiles.map((f, i) => (
                              <div key={i} style={{ width: 56, height: 56, borderRadius: 12, overflow: "hidden", border: `1px solid ${colors.border}` }}>
                    ...
                    {p.damage_photos && p.damage_photos.length > 0 && (
                      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 14 }}>
                        {p.damage_photos.map((url, i) => (
                          <a key={i} href={url} target="_blank" rel="noopener" style={{ width: 80, height: 80, borderRadius: 12, overflow: "hidden", border: `1px solid ${colors.border}`, display: "block" }}>
                  ...
                  <div style={{ textAlign: "center", padding: 10 }}>
                    ...
                    {p.damage_photos && p.damage_photos.length > 0 && (
                      <div style={{ display: "flex", gap: 6, justifyContent: "center", flexWrap: "wrap" }}>
                        {p.damage_photos.map((url, i) => (
                          <a key={i} href={url} target="_blank" rel="noopener" style={{ width: 56, height: 56, borderRadius: 12, overflow: "hidden", border: `1px solid ${colors.border}`, display: "block" }}>
                            <img src={url} alt={`Schaden ${i + 1}`} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                          </a>
                        ))}
                      </div>
                ...
                {isRental && isSeller && p.status === "returned" && (
                  <div>
                    ...
                    <a href={`/order/${p.id}/invoice?type=deposit`} target="_blank" rel="noopener" style={{ display: "block", textAlign: "center", padding: 14, borderRadius: 12, border: `1.5px solid #50804F`, background: "#E8F5E9", color: "#50804F", fontSize: 14, fontWeight: 700, textDecoration: "none", fontFamily: fonts.body, marginBottom: 10 }}>Kautions-Rechnung (QR) ansehen</a>
                  ...
                  <div style={{ textAlign: "center", padding: 10 }}>
                    ...
                    {depositAmount > 0 && (
                      <>
                        ...
                        <div style={{ padding: 12, background: K.sand, borderRadius: 12, border: `1px solid ${K.ink}22`, marginBottom: 12, fontSize: 13, textAlign: "left" }}>
                          <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}><span>Kaution</span><span>CHF {fmtCHF(depositAmount)}</span></div>
                          {parseFloat(p.damage_amount || 0) > 0 && (
                            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, color: "#c62828" }}><span>Schaden</span><span>- CHF {fmtCHF(p.damage_amount)}</span></div>
                          )}
                        ...
                        <a href={`/order/${p.id}/invoice?type=deposit`} target="_blank" rel="noopener" style={{ display: "block", textAlign: "center", padding: 12, borderRadius: 12, border: `1.5px solid #50804F`, background: "#E8F5E9", color: "#50804F", fontSize: 13.5, fontWeight: 700, textDecoration: "none", fontFamily: fonts.body }}>Kautions-Rechnung (QR) ansehen</a>
                      </>
                    )}
                  </div>
                )}
          ...
          <div>
            ...
            <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E5E8EC", padding: 20, marginBottom: 16 }}>
              ...
              <SidebarSection icon={User} title={counterpartLabel}>
                ...
                <button className="eckig kein-akzent" onClick={handleChat} disabled={chatOeffnet}
                  style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, width: "100%", marginTop: 10, padding: "10px 14px", borderRadius: 12, border: "1px solid #E5E8EC", background: "#fff", color: "#191615", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: fonts.body, opacity: chatOeffnet ? .6 : 1 }}>
            ...
            {isBuyer && p.seller?.iban && (
              <div id="payment-info" style={{ background: "#fff", borderRadius: 12, border: "1px solid #E5E8EC", padding: 20, marginBottom: 16 }}>
                ...
                <Link href={`/order/${p.id}/invoice`} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 16, padding: "12px 16px", borderRadius: 12, background: K.petrol, color: "#fff", fontSize: 13, fontWeight: 700, textDecoration: "none", fontFamily: fonts.body }}><FileText size={15} /> QR-Rechnung anzeigen</Link>
              </div>
            )}
