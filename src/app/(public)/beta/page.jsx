...
const TESTS = [
  {
    ...
    items: [
      ...
      { id: "nav_bottom_meeko", label: "Handy: Leiste unten mit B (Home), Lupe, dunklem Plus, gedrehtem B (Favoriten), Profil; aktiver Reiter auf Butter-Pille; eingeloggt zeigt der Kopf nur das B" },
  ...
  },
  {
    ...
    items: [
      ...
      { id: "sell_auto_freigabe", label: "Inserat einreichen: unter Meine Inserate steht 'In Prüfung', innert etwa einer Minute wird es von selbst 'Aktiv' (Glocke: 'Dein Inserat ist live', evtl. mit Tipp). Inserat mit WhatsApp-Nummer im Text bleibt in 'Persönliche Prüfung'" },
      { id: "sell_auktion_gesperrt", label: "Auktion mit Gebot bearbeiten: gelber Hinweis oben, Titel/Preise/Dauer/Typ/Kategorie/Zustand/Bee-Rate ausgegraut (grauer Hintergrund, nicht klickbar), Beschreibung nur lesbar mit Feld 'Nachtrag' (landet mit Datum unter dem Text), Fotos entfernen gesperrt, neue Fotos gehen; auf Meine Inserate heisst der Knopf 'Eingeschränkt'" },
  ...
  },
  {
    ...
    items: [
      ...
      { id: "sr_ki_schalter", label: "Suchfeld: Schalter 'KI an/aus' (Desktop rechts im Feld, mobil in der Suchzeile der Suchseite). An: Rand und Suchen-Knopf petrol, Platzhalter 'Beschreib, was du suchst', Enter geht in die KI-Suche. Aus: gelb, Wortsuche. Einstellung bleibt nach Neuladen erhalten" },
      { id: "sr_ki_auto", label: "Suche mit KI aus: Text ohne wörtliche Treffer (z. B. 'retro games') zeigt 'Nichts gefunden' mit Knopf 'Mit KI suchen', der das KI-Panel öffnet, nach der Bedeutung sucht und das in der Hinweiszeile erklärt; kein gelber Schein mehr beim Klick ins Suchfeld" },
      { id: "sr_ki_header", label: "Header-Suchfeld mit KI an: Suchen-Knopf und Enter öffnen die KI-Suche auf der Suchseite und nehmen den getippten Text mit" },
  ...
  },
  {
    ...
    items: [
      ...
      { id: "auk_erstes_gebot", label: "Erstes Gebot: Startpreis reicht (kein +1 nötig); danach steht die Auktion auf dem Startpreis, das eigene Maximum ist als Preislimit sichtbar; Dialog erklärt das" },
  ...
  },
  {
    ...
    items: [
      ...
      { id: "acc_shop_banner", label: "Shop-Banner (Unternehmen): In den Einstellungen auf Konto-Typ Unternehmen wechseln, Banner hochladen; es erscheint über dem öffentlichen Profil (4:1 beschnitten), Ersetzen und Entfernen funktionieren" },
  ...
  },
  {
    ...
    items: [
      ...
      { id: "bf_sofortkauf_auktion", label: "Sofortkauf einer laufenden Auktion: Bestellung, Rechnung, QR-Zahlteil, Gebühr und Verkaufsmail zeigen alle den Sofortkauf-Preis (nicht den Gebotsstand); Lieferzeile steht auf der Rechnung (Abholung bzw. Versandart)" },
  ...
  },
  {
    ...
    items: [
      ...
      { id: "home_fakten_chips", label: "Startseite: mittige Fakten-Zeile unter dem Hero, vier Schnelleinstiege vorne in der Kategorie-Reihe (weiss mit Rand, dann ein Trennstrich, dann die Kategorien). 'Unter CHF 20' öffnet die Suche mit gesetzter Preisgrenze, 'Endet bald' zeigt Auktionen nach Restzeit, 'Mit Versand' setzt den Lieferfilter" },
      { id: "preis_gesenkt", label: "Festpreis senken (der alte Preis stand mindestens 24 Stunden): Karte und Inseratseite zeigen den alten Preis durchgestrichen und die Prozente. Preis wieder erhöhen: Anzeige verschwindet. Hochsetzen und sofort senken: keine Anzeige" },
  ...
  },
  {
    ...
    items: [
      ...
      { id: "hive_poker_lobby", label: "Poker-Lobby (/poker): Tisch eröffnen (Name, Buy-in 20/50/100, Plätze), Buy-in wird sofort abgezogen, Tisch erscheint unter Meine Tische; fremde offene Tische unter Offene Tische mit Hinsetzen; Tisch startet automatisch, wenn voll, oder der Ersteller startet ab 2 Spielern" },
      { id: "hive_poker_hand", label: "Poker-Tisch: eigene zwei Karten sichtbar, fremde nicht; Statuszeile zeigt wer am Zug ist und die Restzeit; Passen/Schieben/Mitgehen/Erhöhen auf/All-in nur wenn man dran ist, zu kleines Erhöhen wird abgelehnt; Board füllt sich Flop/Turn/River; Ergebnis der letzten Hand mit aufgedeckten Karten und Blattname; Aufstehen schreibt Chips als Pollen gut (im Pollen-Verlauf: Poker Buy-in / Aufgestanden)" },
  ...
  },
  {
    ...
    items: [
      ...
      { id: "adm_kom_bereiche", label: "Kommunikations-Tab neu in Bereichen (Banner / Laufschrift / Rundruf / Historie): immer ein Bereich in voller Breite, Laufschrift-Vorschau ist 1:1 die echte Laufschrift, Historie mit Filter-Pills" },
      { id: "adm_ticker_tempo", label: "Laufschrift-Tempo: drei Stufen (Langsam/Normal/Schnell) wählbar, Vorschau übernimmt sofort; Tempo bleibt gleich, egal ob der Text kurz oder lang ist" },
      ...
      { id: "ins_ki_multi", label: "KI liest mehrere Fotos: mehrere Bilder hochladen (z.B. Rückseite mit Etikett, Detailfoto mit Kratzer), 'Mit KI ausfüllen' drücken; Details aus den Zusatzfotos tauchen in Beschreibung und Zustand auf" },
      { id: "ins_ki_hinweis", label: "KI-Hinweisfeld: vor 'Mit KI ausfüllen' einen Stichwort-Hinweis eingeben (z.B. 'aus MDF, Marke Lista'); Material/Marke/Modell aus dem Hinweis landen korrekt in Titel und Beschreibung" },
  ...
  },
  {
    ...
    items: [
      ...
      { id: "ds_fonts", label: "Schriften korrekt (Instrument Sans überall, Logo-Wortmarke in Sora)" },
      { id: "ds_colors", label: "Farben konsistent (dünne schwarze Ränder, Pastellflächen, Hauptknöpfe dunkel oder Butter, nirgends mehr Teal oder Grau)" },
      { id: "ds_meeko_logo", label: "Neues Logo (kacheliges B mit beedaro) in Header, Fuss, Login, Rechnung; Browser-Tab und App-Icon zeigen das B auf Mint" },
      { id: "ds_meeko_header", label: "Header als schwebende Pille: Suche, KI-Schalter, Kategorien, Menüs, Glocke, Chat, Profil funktionieren; beim Hovern erscheint eine Lavendel-Fläche" },
      { id: "ds_meeko_hero", label: "Startseite: Inserate schweben links und rechts vom Hauptsatz, überschneiden nichts, weichen der Maus aus, bei jedem Laden andere" },
      { id: "ds_meeko_card", label: "Inserat-Karten überall: Foto auf Pastelltafel in der Formatfarbe (Festpreis Rosé, Auktion Lavendel, Miete Himmel, Gratis Butter, Service Rosa), Herz ist das gedrehte B" },
      { id: "ds_meeko_form", label: "Inserieren: gewählte Format-Kachel farbig, Schalter und gewählte Bee-Impact-Stufe in der Formatfarbe, Abstände gleichmässig, nichts eckig, Vorschau zeigt die echte Karte" },
      { id: "ds_meeko_flow", label: "Kaufen, Bieten, Mieten, Buchen, Bestellung, Chat funktionieren wie vor dem neuen Design (nur das Aussehen hat sich geändert)" },
      { id: "ds_buttons_pill", label: "Knöpfe durchgehend rund oder mit 10-px-Ecken und eingedrücktem Schatten, keine eckigen Kästen" },
      { id: "ds_cards_round", label: "Karten und Boxen: Rundung 20, schwarzer Rand, kein Schlagschatten (überall gleich)" },
                  ...
                  {topMelder.slice(0, 5).map((r, i) => (
                    <div key={r.melder} style={{ display: "flex", alignItems: "center", gap: 10, padding: "4px 0", borderBottom: i < Math.min(topMelder.length, 5) - 1 ? "1px solid rgba(29,29,29,.08)" : "none" }}>
                      <span style={{
                        ...
                        background: i === 0 ? colors.butter : i === 1 ? "#F6F4EF" : "transparent",
