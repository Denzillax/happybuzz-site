    ...
    <div style={{ fontFamily: fonts.body, background: K.paper, minHeight: "100vh", color: K.ink }}>
      <div className="bd-seite">
        ...
        <Card style={{ background: `linear-gradient(135deg, ${level.color}14, #fff 60%)`, marginBottom: 16 }}>
          ...
          {/* Progress */}
          <div style={{ height: 12, borderRadius: 12, background: colors.borderLt, overflow: "hidden" }}>
            <div style={{ height: "100%", borderRadius: 12, background: level.color, width: `${next ? progress : 100}%`, transition: "width .6s ease-out" }} />
          </div>
            ...
            {NEKTAR_CATALOG.map(r => {
              ...
              return (
                <div key={r.key} style={{
                  ...
                  {affordable ? (
                    <button onClick={() => startRedeem(r)} style={{ marginTop: 6, padding: "8px 0", borderRadius: 12, border: "none", background: colors.yellow, color: colors.dark, fontSize: 12, fontWeight: 800, fontFamily: fonts.body, cursor: "pointer" }}>Einlösen</button>
                  ) : (
                    <button disabled style={{ marginTop: 6, padding: "8px 0", borderRadius: 12, border: `1px solid ${colors.borderLt}`, background: colors.cream, color: colors.mutedLt, fontSize: 12, fontWeight: 700, fontFamily: fonts.body, cursor: "default" }}>Noch {(r.cost - nektar).toLocaleString("de-CH")} Nektar nötig</button>
                  )}
                </div>
              );
            })}
          ...
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 12, padding: "24px 26px", maxWidth: 380, width: "100%", fontFamily: fonts.body }}>
            ...
            {redeemReward.needsListing && (
              pickerListings === null ? (
                ...
                <select value={pickedListing || ""} onChange={(e) => setPickedListing(e.target.value)}
                  style={{ width: "100%", padding: "11px 12px", borderRadius: 12, border: `1.5px solid ${colors.border}`, fontSize: 14, fontFamily: fonts.body, marginBottom: 14, background: "#fff", outline: "none" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={confirmRedeem} disabled={redeeming || (redeemReward.needsListing && (!pickerListings?.length || !pickedListing))}
                style={{ flex: 1, padding: 13, borderRadius: 12, border: "none", background: colors.yellow, color: colors.dark, fontSize: 14, fontWeight: 800, fontFamily: fonts.body, cursor: "pointer", opacity: (redeeming || (redeemReward.needsListing && (!pickerListings?.length || !pickedListing))) ? 0.5 : 1 }}>
                {redeeming ? "Einlösen…" : "Einlösen"}
              </button>
              <button onClick={() => setRedeemReward(null)} disabled={redeeming} style={{ padding: "13px 18px", borderRadius: 12, border: `1.5px solid ${colors.border}`, background: "#fff", color: colors.muted, fontSize: 13, cursor: "pointer", fontFamily: fonts.body }}>Abbrechen</button>
            </div>
          </div>
        </div>
      )}
