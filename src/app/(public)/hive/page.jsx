...
function Card({ children, style }) {
  return (
    <div style={{ background: "#fff", borderRadius: 20, border: "1px solid #1D1D1D", padding: "20px 22px", ...style }}>
      {children}
    </div>
  );
}
    ...
    <div style={{ fontFamily: fonts.body, background: "var(--bd-grund)", minHeight: "100vh", color: K.ink }}>
      <div className="bd-seite">
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <BeeIcon size={26} color={colors.dark} />
        ...
        <Card style={{ background: `linear-gradient(135deg, ${level.color}14, #fff 60%)`, marginBottom: 16 }}>
          ...
          {/* Progress */}
          <div style={{ height: 12, borderRadius: 20, background: colors.borderLt, overflow: "hidden" }}>
            <div style={{ height: "100%", borderRadius: 20, background: level.color, width: `${next ? progress : 100}%`, transition: "width .6s ease-out" }} />
          </div>
          ...
          <div style={{
            marginTop: 16, padding: "12px 14px", borderRadius: 20,
            ...
            {convertiblePollen >= 1 ? (
              <button onClick={convertAll} disabled={converting} style={{
                width: "100%", marginTop: 10, padding: "9px 0", borderRadius: 20, border: "none",
          ...
          <div style={{
            ...
            borderRadius: 20, background: "#E8A82014", border: `1px solid #E8A82033`,
          }}>
        ...
        <Card style={{ marginBottom: 16 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(220px, 1fr))", gap: 10 }}>
            {NEKTAR_CATALOG.map(r => {
              ...
              return (
                <div key={r.key} style={{
                  padding: "12px 14px", borderRadius: 20, border: `1px solid ${affordable ? "#E8A82055" : colors.borderLt}`,
                  ...
                  {affordable ? (
                    <button onClick={() => startRedeem(r)} style={{ marginTop: 6, padding: "8px 0", borderRadius: 20, border: "none", background: colors.butter, color: colors.dark, fontSize: 12, fontWeight: 800, fontFamily: fonts.body, cursor: "pointer" }}>Einlösen</button>
                  ) : (
                    <button disabled style={{ marginTop: 6, padding: "8px 0", borderRadius: 20, border: `1px solid ${colors.borderLt}`, background: colors.cream, color: colors.mutedLt, fontSize: 12, fontWeight: 700, fontFamily: fonts.body, cursor: "default" }}>Noch {(r.cost - nektar).toLocaleString("de-CH")} Nektar nötig</button>
                  )}
                </div>
              );
            })}
              ...
              {challenges.map(c => {
                ...
                return (
                  <div key={c.id}>
                    ...
                    <div style={{ height: 8, borderRadius: 20, background: colors.borderLt, overflow: "hidden" }}>
                      <div style={{ height: "100%", borderRadius: 20, background: c.done ? "#50804F" : colors.teal, width: `${pct}%`, transition: "width .5s" }} />
                    </div>
              ...
              {leaderboard.map(row => {
                ...
                return (
                  <div key={row.userId} style={{
                    display: "flex", alignItems: "center", gap: 12, padding: "9px 10px", borderRadius: 20,
      ...
      {redeemReward && (
        <div onClick={() => !redeeming && setRedeemReward(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", zIndex: 10000, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, padding: "24px 26px", maxWidth: 380, width: "100%", fontFamily: fonts.body }}>
            ...
            {redeemReward.needsListing && (
              pickerListings === null ? (
                ...
                <select value={pickedListing || ""} onChange={(e) => setPickedListing(e.target.value)}
                  style={{ width: "100%", padding: "11px 12px", borderRadius: 20, border: `1.5px solid ${colors.border}`, fontSize: 14, fontFamily: fonts.body, marginBottom: 14, background: "#fff", outline: "none" }}>
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={confirmRedeem} disabled={redeeming || (redeemReward.needsListing && (!pickerListings?.length || !pickedListing))}
                style={{ flex: 1, padding: 13, borderRadius: 20, border: "none", background: colors.butter, color: colors.dark, fontSize: 14, fontWeight: 800, fontFamily: fonts.body, cursor: "pointer", opacity: (redeeming || (redeemReward.needsListing && (!pickerListings?.length || !pickedListing))) ? 0.5 : 1 }}>
                {redeeming ? "Einlösen…" : "Einlösen"}
              </button>
              <button onClick={() => setRedeemReward(null)} disabled={redeeming} style={{ padding: "13px 18px", borderRadius: 20, border: `1.5px solid ${colors.border}`, background: "#fff", color: colors.muted, fontSize: 13, cursor: "pointer", fontFamily: fonts.body }}>Abbrechen</button>
            </div>
          </div>
        </div>
      )}
