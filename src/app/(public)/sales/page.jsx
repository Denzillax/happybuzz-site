  ...
  return (
    <div style={{ fontFamily: fonts.body, background: K.paper, minHeight: "100vh", color: K.ink }}>
      <div style={{ maxWidth: 1000, margin: "0 auto", padding: "32px 20px 80px" }}>
        ...
        {/* Tabelle (Desktop) */}
        {!loading && filtered.length > 0 && (
          <div className="po-table" style={{ background: "#fff", borderRadius: 12, border: "1px solid #E4E0D8", overflow: "hidden" }}>
                ...
                {filtered.map(s => {
                  ...
                  return (
                    <tr key={s.id} style={{ borderBottom: `1px solid ${colors.borderLt}`, transition: "background .1s" }}
                      ...
                      <td style={{ padding: "14px 10px", verticalAlign: "middle" }}>
                        {s.shipping_method && <div>{shippingMethodLabel(s.shipping_method)}</div>}
        ...
        {!loading && filtered.length > 0 && (
          <div className="po-cards" style={{ background: "#fff", borderRadius: 12, border: "1px solid #E4E0D8" }}>
            {filtered.map(s => {
              const st = STATUS_CONFIG[s.status] || STATUS_CONFIG.confirmed;
              const StIcon = st.icon;
              const ref = makeBeeRef(s.id);
        ...
        {!loading && sales.length > 0 && (
          <div style={{ display: "flex", gap: 12, marginTop: 20, flexWrap: "wrap" }}>
            <div style={{ flex: 1, minWidth: 200, background: "#fff", borderRadius: 12, border: "1px solid #E4E0D8", padding: "16px 20px" }}>
            ...
            <div style={{ flex: 1, minWidth: 200, background: "#EEF4EC", borderRadius: 12, border: "1px solid #E4E0D8", padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
