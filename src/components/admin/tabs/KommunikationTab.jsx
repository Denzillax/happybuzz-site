...
const Toggle = ({ on, onChange }) => (
  <button onClick={onChange} style={{ width: 46, height: 26, borderRadius: 12, border: "1px solid #E5E8EC", cursor: "pointer", background: on ? colors.yellow : "#E5E8EC", position: "relative", transition: "background .2s", flexShrink: 0, padding: 0 }}>
    <div style={{ width: 18, height: 18, background: on ? INK : "#fff", border: "1px solid #E5E8EC", position: "absolute", top: 2, left: on ? 23 : 2, transition: "left .2s", boxSizing: "border-box" }} />
  </button>
);

// Eckige Wahl-Knoepfe im Katalog-Stil: aktiv = Ink auf Gelb
const Segment = ({ options, value, onChange }) => (
  <div style={{ display: "inline-flex", flexWrap: "wrap", maxWidth: "100%", gap: 0, border: "1px solid #E5E8EC" }}>
    {options.map(([k, l], i) => (
      <button key={k} onClick={() => onChange(k)} style={{
        fontSize: 11.5, fontWeight: 700, padding: "8px 14px", border: "none",
        borderLeft: i > 0 ? "1px solid #E5E8EC" : "none",
...
const ColorPick = ({ bg, text, onPick, onBg, onText }) => (
  <div style={{ display: "flex", gap: 14, alignItems: "center", flexWrap: "wrap" }}>
    <div style={{ display: "inline-flex", border: "1px solid #E5E8EC" }}>
      {ANNOUNCEMENT_PRESETS.map((p, i) => (
        <button key={p.name} onClick={() => onPick(p)} title={p.name} style={{
          ...
          border: "none", borderLeft: i > 0 ? "1px solid #E5E8EC" : "none", padding: 0,
        }}>
    ...
    {[["Hintergrund", bg, onBg], ["Schrift", text, onText]].map(([l, val, on]) => (
      <label key={l} style={{ display: "inline-flex", alignItems: "center", gap: 7, cursor: "pointer" }}>
        <input type="color" value={/^#[0-9a-fA-F]{6}$/.test(val) ? val : "#000000"} onChange={e => on(e.target.value)}
          style={{ width: 34, height: 34, padding: 0, border: "1px solid #E5E8EC", borderRadius: 12, cursor: "pointer", background: "none" }} />
...
const Panel = ({ children }) => (
  <div style={{ background: "#fff", border: "1px solid #E5E8EC", padding: "22px 22px 24px", display: "flex", flexDirection: "column", gap: 18 }}>
    {children}
  </div>
);
...
const PreviewBlock = ({ on, children }) => (
  <div>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 8 }}>
      ...
      <span style={{ fontFamily: MONO, fontSize: 9.5, fontWeight: 700, letterSpacing: ".08em", textTransform: "uppercase", padding: "3px 9px", border: "1px solid #E5E8EC", background: on ? colors.yellow : "#fff", color: INK }}>{on ? "Aktiv" : "Aus"}</span>
    </div>
    {children}
  </div>
);
...
const SaveBtn = ({ onClick, label }) => (
  <button onClick={onClick} style={{
    ...
    border: "1px solid #E5E8EC", borderRadius: 12, padding: "12px 30px",
      ...
      {bereich === "banner" && (
        <Panel>
          <PreviewBlock on={ann.enabled}>
            <div style={{ background: ann.bg_color, color: ann.text_color, fontSize: 15, fontWeight: 700, padding: "12px 14px", textAlign: "center", overflow: "hidden", whiteSpace: "nowrap", textOverflow: "ellipsis", border: "1px solid #E5E8EC", opacity: ann.enabled ? 1 : 0.5 }}>
          ...
          <div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "flex-end" }}>
            <Field label="Status">
              <div style={{ display: "flex", alignItems: "center", gap: 10, height: 38 }}>
                ...
                <span style={{ fontSize: 12.5, fontWeight: 600, color: ann.enabled ? "#1D1D1D" : colors.muted }}>{ann.enabled ? "Für alle sichtbar" : "Aus"}</span>
              </div>
            </Field>
            <Field label="Text" grow>
              <input value={ann.message} onChange={e => setAnn({ ...ann, message: e.target.value })} placeholder="Text des Balkens…" style={{ ...bcInput, borderRadius: 12, border: "1px solid #E5E8EC", maxWidth: 620 }} />
            </Field>
          </div>
          <Field label="Farbe">
          ...
          <div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "flex-end" }}>
            <Field label="Status">
              <div style={{ display: "flex", alignItems: "center", gap: 10, height: 38 }}>
                ...
                <span style={{ fontSize: 12.5, fontWeight: 600, color: ticker.enabled ? "#1D1D1D" : colors.muted }}>{ticker.enabled ? "Für alle sichtbar" : "Aus"}</span>
              </div>
            </Field>
            <Field label="Text" grow>
              <input value={ticker.message} onChange={e => setTicker({ ...ticker, message: e.target.value })} placeholder="Text der Laufschrift…" style={{ ...bcInput, borderRadius: 12, border: "1px solid #E5E8EC", maxWidth: 620 }} />
            </Field>
          </div>
          <Field label="Farbe">
              ...
              {histRows.map((h, i) => {
                ...
                return (
                  <div key={i} style={{ display: "grid", gridTemplateColumns: "58px 130px 1fr", columnGap: 10, alignItems: "start", padding: "8px 0", borderBottom: i < histRows.length - 1 ? "1px solid rgba(20,17,13,.08)" : "none", fontSize: 12.5 }}>
                    <span style={{ fontFamily: MONO, fontSize: 9.5, fontWeight: 700, letterSpacing: ".06em", padding: "2px 0", textAlign: "center", background: art === "RUF" ? "#1D1D1D" : colors.yellow, color: art === "RUF" ? "#fff" : colors.dark }}>{art}</span>
