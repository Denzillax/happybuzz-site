          ...
          <div style={{ display: "grid", gridTemplateColumns: g.items.length > 1 ? "1fr 1fr" : "1fr", gap: 10 }}>
            {g.items.map(([k, label]) => (
              <label key={k} style={{ fontSize: 12, fontWeight: 600, color: colors.dark }}>
                {label}
                <input value={form[k] || ""} onChange={e => set(k, e.target.value)} style={{ width: "100%", marginTop: 4, border: `1px solid ${k === "iban" && !ibanOk ? "#EB5E55" : colors.border}`, borderRadius: 10, padding: "9px 11px", fontSize: 13, fontFamily: fonts.body, outline: "none", boxSizing: "border-box" }} />
