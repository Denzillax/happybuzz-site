  ...
  return (
    <div>
      ...
      <div style={{ background: "#fff", border: `1px solid ${colors.border}`, borderRadius: radius.lg, padding: "12px 14px", marginBottom: 14 }}>
        ...
        <textarea value={note} onChange={e => setNote(e.target.value)} rows={3} placeholder="Interne Notiz zu diesem Nutzer…" style={{ width: "100%", border: `1px solid ${colors.border}`, borderRadius: 12, padding: "8px 10px", fontSize: 13, fontFamily: fonts.body, resize: "vertical", outline: "none", boxSizing: "border-box" }} />
        ...
        {profileAudit.length === 0 ? <div style={{ fontSize: 12, color: colors.muted }}>Keine Aktionen protokolliert.</div> : profileAudit.map(a => {
          ...
          return (
            <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0", borderBottom: `1px solid ${colors.borderLt}` }}>
              <span style={{ width: 28, height: 28, borderRadius: 12, background: meta.bg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Icon size={14} color={meta.color} /></span>
        ...
        {(userTab[u.id] || "inserate") === "inserate" && (
          <div style={{ padding: "10px 16px" }}>
            {uLst.length > 0 ? uLst.map(l => (
              <div key={l.id} className="adm-lrow" style={{ display: "grid", gridTemplateColumns: "1fr 80px 90px 80px 80px", alignItems: "center", gap: 6, padding: "8px 0", borderBottom: `1px solid ${colors.borderLt}` }}>
        ...
        {(userTab[u.id] || "inserate") === "bestellungen" && (
          <div style={{ padding: "10px 16px" }}>
            {uOrders.length > 0 ? uOrders.map(o => {
              ...
              return (
                <div key={o.id} className="adm-orow" style={{ display: "grid", gridTemplateColumns: "70px 1fr 90px 80px 70px", alignItems: "center", gap: 6, padding: "7px 0", borderBottom: `1px solid ${colors.borderLt}`, opacity: o.status === "cancelled" ? 0.5 : 1 }}>
