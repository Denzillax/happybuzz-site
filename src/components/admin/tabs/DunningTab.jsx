...
export function DunningTab({ admin }) {
  ...
  const card = (inv, opts = {}) => (
    <div key={inv.id} style={{ marginBottom: 10, borderRadius: radius.lg, border: `1px solid ${opts.paused ? colors.border : "#f0c9c9"}`, overflow: "hidden", padding: "14px 16px", background: opts.paused ? "#fff" : "#FFEBEE" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 11 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 11, color: "#C62828", fontWeight: 600 }}>CHF {fmtCHF(inv.total_fees)} · {opts.paused ? "alle Stufen gesendet · Inserate pausiert" : `fällig seit ${daysOverdue(inv)} Tagen`}</div>
        </div>
  ...
  return (
    <div>
      ...
      {overdueInvoices.length === 0 ? (
        ...
        <>
          {dunningDue.length > 0 && groupHeader("Jetzt fällig", "#C62828")}
