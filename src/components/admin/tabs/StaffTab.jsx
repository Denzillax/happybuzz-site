        ...
        {list.slice(0, 50).map(u => {
          ...
          return (
            <div key={u.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 16px", borderBottom: `1px solid ${colors.borderLt}` }}>
              ...
              <select value={role} onChange={e => setStaffRole(u.id, e.target.value)} style={{ border: `1px solid ${colors.border}`, borderRadius: 8, padding: "7px 10px", fontSize: 12, fontFamily: fonts.body, background: "#fff", cursor: "pointer", flexShrink: 0 }}>
