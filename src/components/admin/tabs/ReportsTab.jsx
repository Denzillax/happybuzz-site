  ...
  return (
    <div style={{ background: colors.surface, borderRadius: radius.lg, border: `1px solid ${colors.border}`, overflow: "hidden" }}>
      {reports.length === 0 ? (
        ...
        return (
        <div key={r.id} style={{ padding: "13px 16px", borderBottom: `1px solid ${colors.borderLt}`, background: closed ? "#F5F6F8" : "transparent", opacity: st === "abgelehnt" ? 0.55 : closed ? 0.7 : 1 }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4, flexWrap: "wrap" }}>
                ...
                <select value={st} onChange={e => setReportStatus(r.id, e.target.value)}
                  style={{ border: `1px solid ${colors.border}`, borderRadius: 12, padding: "4px 8px", fontSize: 11, fontFamily: fonts.body, background: "#fff", cursor: "pointer", flexShrink: 0 }}>
              ...
              {r.description && <p style={{ margin: "0 0 6px", fontSize: 12, color: "#5B626C" }}>{r.description}</p>}
              {/* Aktionen */}
              {!closed && (
