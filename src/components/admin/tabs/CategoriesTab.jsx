        ...
        {ICON_CHOICES.map(name => {
          ...
          return (
            <button key={name} onClick={() => { onPick(name); onClose(); }} title={name}
              style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: 7, borderRadius: 12, cursor: "pointer",
          ...
          {isEditing ? (
            <span style={{ display: "inline-flex", alignItems: "center", gap: 5, flex: 1, minWidth: 0 }}>
              <input autoFocus value={editName} onChange={e => setEditName(e.target.value)}
                ...
                style={{ flex: 1, minWidth: 0, border: `1px solid ${colors.teal}`, borderRadius: 12, padding: "4px 8px", fontSize: 13, fontFamily: fonts.body, outline: "none" }} />
              <button onClick={commitEdit} title="Speichern" style={{ border: "none", background: "#E8F5E9", color: "#2E7D32", borderRadius: 12, padding: 4, cursor: "pointer", lineHeight: 0 }}><Check size={13} /></button>
              <button onClick={() => setEditId(null)} title="Abbrechen" style={{ border: "none", background: colors.cream, color: colors.muted, borderRadius: 12, padding: 4, cursor: "pointer", lineHeight: 0 }}><X size={13} /></button>
            </span>
          ) : (
          ...
          {!isEditing && confirmDelId === cat.id && (
            <span style={{ display: "inline-flex", alignItems: "center", gap: 6, flexShrink: 0 }}>
              ...
              <button onClick={async () => { const ok = await deleteCategory(cat); setConfirmDelId(null); }} title="Endgültig löschen"
                style={{ border: "none", background: "#c62828", color: "#fff", borderRadius: 12, padding: "4px 10px", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: fonts.body }}>Ja</button>
              <button onClick={() => setConfirmDelId(null)} title="Abbrechen"
                style={{ border: "none", background: colors.cream, color: colors.muted, borderRadius: 12, padding: 4, cursor: "pointer", lineHeight: 0 }}><X size={13} /></button>
            </span>
          )}
          {!isEditing && confirmDelId !== cat.id && (
            <span style={{ display: "inline-flex", gap: 4, flexShrink: 0 }}>
              <button onClick={() => startEdit(cat)} title="Umbenennen" style={{ border: "none", background: colors.cream, color: colors.muted, borderRadius: 12, padding: 5, cursor: "pointer", lineHeight: 0 }}><Pencil size={12} /></button>
              <button onClick={() => { setAddingUnder(addingUnder === cat.id ? null : cat.id); setSubName(""); }} title="Unterkategorie anlegen" style={{ border: "none", background: colors.cream, color: colors.muted, borderRadius: 12, padding: 5, cursor: "pointer", lineHeight: 0 }}><Plus size={12} /></button>
              <button onClick={() => toggleCategoryActive(cat)} title={inactive ? "Aktivieren" : "Deaktivieren"}
                style={{ border: "none", background: inactive ? "#E8F5E9" : "#FFF3E0", color: inactive ? "#2E7D32" : "#E65100", borderRadius: 12, padding: 5, cursor: "pointer", lineHeight: 0 }}>
                {inactive ? <Eye size={12} /> : <EyeOff size={12} />}
              </button>
              <button onClick={() => { setConfirmDelId(cat.id); }} title="Löschen"
                style={{ border: "none", background: "#FFEBEE", color: "#c62828", borderRadius: 12, padding: 5, cursor: "pointer", lineHeight: 0 }}><Trash2 size={12} /></button>
            </span>
          )}
        </div>
        {/* Neue Unterkategorie */}
        {addingUnder === cat.id && (
          <div style={{ display: "flex", gap: 6, padding: "8px 14px", paddingLeft: 14 + (depth + 1) * 26, borderBottom: `1px solid ${colors.borderLt}`, background: "#FBFAF7" }}>
            ...
            <button onClick={() => addSub(cat.id, subName)}
              style={{ border: "none", background: colors.dark, color: "#fff", borderRadius: 12, padding: "6px 14px", fontSize: 11, fontWeight: 700, cursor: "pointer", fontFamily: fonts.body }}>Anlegen</button>
          </div>
        )}
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        <span style={{ position: "relative", flexShrink: 0 }}>
          <button onClick={() => setNewIconOpen(v => !v)} title="Icon wählen"
            style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 38, height: 38, border: `1px solid ${colors.border}`, borderRadius: 12, background: "#fff", cursor: "pointer", color: colors.dark }}>
        ...
        <input value={newName} onChange={e => setNewName(e.target.value)} placeholder="Neue Hauptkategorie..."
          ...
          style={{ flex: 1, maxWidth: 322, border: `1px solid ${colors.border}`, borderRadius: 12, padding: "8px 12px", fontSize: 13, fontFamily: fonts.body, outline: "none", background: "#fff" }} />
        <button onClick={addMain} disabled={!newName.trim()}
          style={{ display: "inline-flex", alignItems: "center", gap: 6, border: "none", background: newName.trim() ? colors.dark : colors.cream, color: newName.trim() ? "#fff" : colors.muted, borderRadius: 12, padding: "8px 16px", fontSize: 12, fontWeight: 700, cursor: newName.trim() ? "pointer" : "default", fontFamily: fonts.body }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 12, alignItems: "center" }}>
        ...
        <button onClick={expandAll} title="Alles aufklappen" style={{ display: "inline-flex", alignItems: "center", gap: 6, border: `1px solid ${colors.border}`, background: "#fff", color: colors.dark, borderRadius: 12, padding: "8px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: fonts.body }}>
          <Maximize2 size={13} /> Alles auf
        </button>
        <button onClick={collapseAll} title="Alles zuklappen" style={{ display: "inline-flex", alignItems: "center", gap: 6, border: `1px solid ${colors.border}`, background: "#fff", color: colors.dark, borderRadius: 12, padding: "8px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: fonts.body }}>
