    ...
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,.5)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center" }} onClick={() => set("_shipModal", false)}>
      <div onClick={e => e.stopPropagation()} style={{ background: "#fff", borderRadius: 20, width: 480, maxHeight: "80vh", overflow: "auto", padding: 24 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 16 }}>
          {methods.map(m => (
            <button key={m.id} onClick={() => {
              ...
              padding: "10px 14px", borderRadius: 20, border: `1.5px solid ${form.shipping_method === m.id ? colors.dark : colors.border}`,
              background: form.shipping_method === m.id ? `${colors.butter}15` : "transparent",
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
              {weights.map(w => (
                <button key={w} onClick={() => {
                  ...
                  padding: "8px 16px", borderRadius: 20, border: `1.5px solid ${form.ship_weight === w ? colors.dark : colors.border}`,
                  background: form.ship_weight === w ? `${colors.butter}15` : "transparent",
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
          {["economy", "priority"].map(s => (
            <button key={s} onClick={() => {
              ...
              padding: "8px 16px", borderRadius: 20, border: `1.5px solid ${form.ship_speed === s ? colors.dark : colors.border}`,
              background: form.ship_speed === s ? `${colors.butter}15` : "transparent",
              fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: fonts.body,
            }}>{s === "priority" ? "Priority (1 Tag)" : "Economy (2-3 Tage)"}</button>
          ))}
        </div>

        <div style={{ padding: "12px 16px", background: colors.cream, borderRadius: 20, marginBottom: 16 }}>
        ...
        <button onClick={() => {
          ...
          background: colors.butter, color: colors.dark, fontSize: 14, fontWeight: 800,
