  ...
  return (
    <div style={{ background: "#fff", borderRadius: radius.md, border: `1px solid ${colors.border}`, padding: 20, marginBottom: 16 }}>
        ...
        return (
          <div key={ev.id} style={{ display: "flex", gap: 12 }}>
            ...
            <div style={{ paddingBottom: 20, flex: 1 }}>
              ...
              {ev.tracking_number && (
                <a href={`https://service.post.ch/ekp-web/ui/entry/search/${ev.tracking_number}?lang=de`} target="_blank" rel="noopener" style={{ fontSize: 12, color: colors.yellow, fontWeight: 600, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 4, marginTop: 4 }}>{ev.tracking_number} <ExternalLink size={11} /></a>
              )}
              {ev.event_type === "payment_marked" && isBuyer && seller?.iban && (
                <button onClick={() => document.getElementById("payment-info")?.scrollIntoView({ behavior: "smooth" })} style={{ fontSize: 12, color: colors.yellow, fontWeight: 600, background: "none", border: "none", cursor: "pointer", padding: 0, fontFamily: fonts.body, marginTop: 4, display: "block" }}>Bankverbindung des Verkäufers</button>
              )}
            </div>
          </div>
        );
      ...
      {sortedEvents.length > 2 && (
        <button onClick={() => setShowFullTimeline(!showFullTimeline)} style={{ display: "flex", alignItems: "center", gap: 4, background: "none", border: "none", cursor: "pointer", fontSize: 13, fontWeight: 700, color: colors.yellow, fontFamily: fonts.body, padding: "4px 0 0", textTransform: "uppercase", letterSpacing: ".03em" }}>
