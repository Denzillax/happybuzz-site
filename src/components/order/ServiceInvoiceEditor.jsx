...
function ServiceInvoiceEditor({ purchaseId, sellerId, feePercent = 5, onSubmitted }) {
  ...
  const inputStyle = {
    padding: "9px 12px", border: "1px solid #1D1D1D", borderRadius: 20,
  ...
  return (
    <div>
      ...
      {items.map((item, idx) => {
        ...
        return (
          <div key={item.id} style={{
            ...
            border: "1px solid #1D1D1D", marginBottom: 8,
          }}>
            {/* Icon */}
            <div style={{
      ...
      <button onClick={handleSubmit} disabled={items.length === 0 || saving || subtotal <= 0} style={{
        width: "100%", padding: "13px 20px", borderRadius: 20, border: "none",
        background: items.length > 0 && subtotal > 0 ? K.petrol : "rgba(29,29,29,.16)",
