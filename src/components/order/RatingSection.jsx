  ...
  return (
    ...
    <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #E5E8EC", padding: 20, marginBottom: 16 }}>
      {myRating || ratingDone ? (
        ...
        <button onClick={() => setShowModal(true)} style={{ width: "100%", padding: 14, borderRadius: 12, border: "1px solid #E5E8EC", background: K.petrol, color: "#fff", fontSize: 14, fontWeight: 800, cursor: "pointer", fontFamily: fonts.body, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
    ...
    {showModal && (
      <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.5)", zIndex: 9999, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }} onClick={() => setShowModal(false)}>
        <div onClick={e => e.stopPropagation()} style={{ background: K.paper, borderRadius: 12, border: "1px solid #E5E8EC", padding: "28px 24px", maxWidth: 420, width: "100%", boxShadow: "0 20px 60px rgba(0,0,0,0.2)" }}>
            ...
            {(isBuyer ? (isService ? TAGS_DIENSTLEISTER : TAGS_VERKAEUFER) : TAGS_KAEUFER).map(t => {
              ...
              return (
                <button key={t} type="button" onClick={() => setSelectedTags(prev => on ? prev.filter(x => x !== t) : [...prev, t])}
                  style={{ padding: "6px 12px", borderRadius: 12, cursor: "pointer", fontFamily: fonts.body, fontSize: 12, fontWeight: on ? 700 : 500,
                    border: "1px solid #E5E8EC", background: on ? K.honey : "#fff", color: K.ink }}>
                  {t}
                </button>
              );
            })}
          </div>
          <textarea value={ratingComment} onChange={e => setRatingComment(e.target.value)} placeholder="Kommentar (optional)" rows={3} style={{ width: "100%", padding: "12px 14px", borderRadius: 12, border: "1px solid #E5E8EC", fontSize: 14, fontFamily: fonts.body, outline: "none", boxSizing: "border-box", resize: "vertical", marginBottom: 14 }} />
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={submitRating} disabled={!rating} style={{ flex: 1, padding: 14, borderRadius: 12, border: "1px solid #E5E8EC", background: rating ? K.honey : "#E5E8EC", color: K.ink, fontSize: 14, fontWeight: 800, cursor: rating ? "pointer" : "default", fontFamily: fonts.body, boxShadow: rating ? "0 2px 8px rgba(25,22,21,.15)" : "none" }}>Bewertung abgeben</button>
            <button onClick={() => setShowModal(false)} style={{ padding: "14px 20px", borderRadius: 12, border: "1px solid #E5E8EC", background: "#fff", color: K.ink, fontSize: 13, cursor: "pointer", fontFamily: fonts.body }}>Abbrechen</button>
          </div>
        </div>
      </div>
    )}
