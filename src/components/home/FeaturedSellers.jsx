    ...
    <section style={{ padding: '48px 24px 0', maxWidth: 1280, margin: '0 auto' }}>
      ...
      <div className="featured-sellers-grid">
        {sellers.map((s) => {
          ...
          return (
            <div key={s.id} style={{ background: '#fff', border: "1px solid #1D1D1D", borderRadius: 20, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              <Link href={`/user/${s.id}`} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 14, textDecoration: 'none', color: 'inherit' }}>
                <div style={{ width: 44, height: 44, borderRadius: '50%', background: SAND, border: "1px solid #1D1D1D", display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
