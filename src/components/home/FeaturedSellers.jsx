    ...
    <section style={{ padding: '48px 24px 0', maxWidth: 1280, margin: '0 auto' }}>
      ...
      <div className="featured-sellers-grid">
        {sellers.map((s) => {
          ...
          return (
            <div key={s.id} style={{ background: '#fff', border: "1px solid #E5E8EC", borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
              <Link href={`/user/${s.id}`} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: 14, textDecoration: 'none', color: 'inherit' }}>
                <div style={{ width: 44, height: 44, borderRadius: '50%', background: SAND, border: "1px solid #E5E8EC", display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', flexShrink: 0 }}>
              ...
              {s.listings.length > 0 ? (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 2, padding: '0 2px 2px' }}>
                  {s.listings.map((l) => {
                    ...
                    return (
                    <Link key={l.id} href={`/listing/${l.id}`} title={l.title} style={{ position: 'relative', aspectRatio: '1 / 1', background: SAND, borderRadius: 12, overflow: 'hidden', textDecoration: 'none', display: 'flex', alignItems: 'flex-end' }}>
                      {l.cover
