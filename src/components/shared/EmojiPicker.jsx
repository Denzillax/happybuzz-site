  ...
  return (
    <div ref={ref} style={{ position: "relative", flexShrink: 0 }}>
      <button type="button" onClick={() => setOffen(o => !o)} title="Emoji einfügen" aria-label="Emoji einfügen" aria-expanded={offen}
        style={{ width: size, height: size, borderRadius: "50%", border: `1.5px solid ${offen ? colors.teal : colors.border}`, background: offen ? "#E6F5F5" : colors.cream, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <Smile size={18} color={offen ? colors.teal : colors.muted} />
      </button>
      {offen && (
        <div style={{ position: "absolute", bottom: size + 8, left: 0, zIndex: 60, width: 8 * 34 + 16, maxWidth: "calc(100vw / var(--bd-zoom, 1) - 28px)", maxHeight: 260, overflowY: "auto", background: "#fff", border: `1px solid ${colors.border}`, borderRadius: 12, boxShadow: "0 8px 24px rgba(0,0,0,.12)", padding: 8 }}>
          {EMOJI_GRUPPEN.map(g => (
