...
export default function NektarBadge() {
  ...
  return (
    <div ref={ref} className="nektar-badge" style={{ position: "relative" }}>
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, zIndex: 200, width: 264, background: "#fff", borderRadius: 10, boxShadow: "0 16px 38px rgba(20,17,13,.16)", border: "1px solid #E4E0D8", padding: 16, fontFamily: "'Manrope', sans-serif" }}>
          ...
          <div style={{ height: 8, borderRadius: 10, background: "#F4F4F2", border: "1px solid rgba(20,17,13,.2)", overflow: "hidden", marginBottom: 6 }}>
            <div style={{ height: "100%", borderRadius: 10, background: level.color, width: `${next ? progress : 100}%`, transition: "width .4s" }} />
          </div>
          ...
          <Link href="/hive" onClick={() => setOpen(false)} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "10px 0", borderRadius: 10, background: "#0E9493", color: "#fff", fontSize: 13, fontWeight: 700, textDecoration: "none" }}>
